pipeline{
    ...
    stages{
        stage("SCM"){
            steps{
                git 'https://github.com/cloudtechmasters/Kubernetes-Deploy-WithJenkinsCICD.git'
            }
        }
        stage("Docker Image"){
            steps{
                sh "docker build -t cloudtechmasters/nodejs-cicd:v1 ."
            }
        }
        stage("Docker Push"){
            steps{
                ...
                sh "docker push cloudtechmasters/nodejs-cicd:v1"
            }
        }
        stage("Deploy"){
            steps{
